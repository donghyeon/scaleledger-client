...
| 18~23 | 예비 | - | Reserved |
| **24~25** | **출력(Relay)** | **Offset ASCII** | **[Control]** 신호등 제어 **(주의: 6.1 인코딩 참조)** |
...
| **19~20** | **출력상태** | **Offset ASCII** | **[Status]** 릴레이 상태 피드백 **(주의: 6.1 인코딩 참조)** |
...
| 2 | ID | ASCII | 장비 번호 (0~9) |
| 3 | CMD | `'P'` | 프린터 출력 커맨드 |
| **4~7** | **전송크기** | ASCII (4) | 인쇄 데이터의 길이 <br>(예: 100 bytes → `"0100"`) |
...
### 6.1. 출력(Relay) 인코딩 (Critical)
Relay 필드는 일반적인 Hex String(0~9, A~F)을 사용하지 않고, **Nibble 값에 ASCII Offset(`+0x30`)을 적용한 특수 인코딩**을 사용합니다.

#### [Encoding/Decoding Rule]
* **Packet Byte = (Nibble Value) + `0x30` (`'0'`)**
* 값 10 이상의 경우, 알파벳 A~F가 아닌 ASCII 테이블 순서대로 매핑됩니다.

| Nibble Value (Dec) | Nibble Value (Hex) | **Encoded Char (ASCII)** | 비고 |
| :---: | :---: | :---: | :--- |
| 0 ~ 9 | `0x0` ~ `0x9` | `'0'` ~ `'9'` | 일반 숫자와 동일 |
| **10** | **`0xA`** | **`:`** | `0x3A` |
| **11** | **`0xB`** | **`;`** | `0x3B` |
| **12** | **`0xC`** | **`<`** | `0x3C` |
| **13** | **`0xD`** | **`=`** | `0x3D` |
...
Relay 값은 아래 비트맵의 합(OR 연산)으로 계산됩니다.

| Bit Position | Value (Hex) | 기능 (Function) | 제어(Request) | 상태(Response) |
| :---: | :---: | :--- | :---: | :---: |
| **Bit 0** | `0x01` | **Green Light** | **ON/OFF** | ON/OFF |
| **Bit 1** | `0x02` | **Red Light** | **ON/OFF** | ON/OFF |
| Bit 2 | `0x04` | Spare | - | - |
| **Bit 3** | `0x08` | **Fan** | N/A | **Status Only** |
| **Bit 4** | `0x10` | **Heater** | N/A | **Status Only** |
| Bit 5~7 | - | Spare | - | - |

#### [계산 예시]
**예시 1: 녹색등(Bit 0) + 팬 작동(Bit 3) 상태인 경우**
1.  **Bit 합산:** `0x01`(Green) | `0x08`(Fan) = **`0x09`** (`0000 1001`)
2.  **Nibble 분리:** High=`0`, Low=`9`
3.  **ASCII 인코딩:**
    * High: `0` + `0x30` = `'0'`
    * Low: `9` + `0x30` = `'9'`
4.  **전송 문자열:** **`"09"`**

**예시 2: 적색등(Bit 1) + 팬 작동(Bit 3) 상태인 경우 (특수 매핑 발생)**
1.  **Bit 합산:** `0x02`(Red) | `0x08`(Fan) = **`0x0A`** (`0000 1010`) (십진수 10)
2.  **Nibble 분리:** High=`0`, Low=`A`(10)
3.  **ASCII 인코딩:**
    * High: `0` + `0x30` = `'0'`
    * Low: `10` + `0x30` = **`':'`** (주의: `'A'`가 아님)
4.  **전송 문자열:** **`"0:"`**
...
3.  **릴레이 인코딩 구현 주의**
    * 표준 `HEX` 파싱 함수(예: `int(x, 16)`)를 사용하면 오류가 발생합니다.
    * 반드시 **(ASCII Byte - 0x30)** 연산을 통해 정수값으로 변환하십시오.
